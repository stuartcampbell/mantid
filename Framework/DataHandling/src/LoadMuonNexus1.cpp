 ...
 */
void LoadMuonNexus1::addPeriodLog(const DataObjects::Workspace2D_sptr& localWorkspace,
                                  int64_t period) {
  auto &run = localWorkspace->mutableRun();
  ISISRunLogs runLogs(run);
  if (period == 0) {
...
void LoadMuonNexus1::addGoodFrames(const DataObjects::Workspace2D_sptr& localWorkspace,
