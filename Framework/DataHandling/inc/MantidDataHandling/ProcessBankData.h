...
public:
  ...
  ProcessBankData(DefaultEventLoader &loader, std::string entry_name,
                  API::Progress *prog, std::shared_ptr<uint32_t[]> event_id,
                  std::shared_ptr<float[]> event_time_of_flight,
                  ...
                  bool have_weight, std::shared_ptr<float[]> event_weight,
...
private:
  ...
  bool have_weight;
  /// event weights array
  std::shared_ptr<float[]> event_weight;
  /// Minimum pixel id
  detid_t m_min_id;
  /// Maximum pixel id
  detid_t m_max_id;
