...
public:
  ...
  createWorkspace(const DataObjects::Workspace2D_sptr& ws_sptr, int64_t nVectors = -1,
...
protected:
  ...
  void runLoadMappingTable(const std::string &fileName,
                           const DataObjects::Workspace2D_sptr&);
  /// load log algorithm
  void runLoadLog(const std::string &fileName, const DataObjects::Workspace2D_sptr&,
                  double, double);
