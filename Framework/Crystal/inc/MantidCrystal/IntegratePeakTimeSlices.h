...
private:
  ...
  void SetUpData(API::MatrixWorkspace_sptr &Data,
                 ...
                 std::shared_ptr<Geometry::IComponent> comp, const int chanMin,
                 const int chanMax, double CentX, double CentY,
                 Kernel::V3D &CentNghbr,

                 double &neighborRadius, // from CentDet
                 double Radius, std::string &spec_idList);

  bool getNeighborPixIDs(std::shared_ptr<Geometry::IComponent> comp,
