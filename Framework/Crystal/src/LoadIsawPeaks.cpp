...
DataObjects::Peak LoadIsawPeaks::readPeak(const PeaksWorkspace_sptr& outWS,
  ...
  int pixelID =
      findPixelID(inst, std::move(bankName), static_cast<int>(col), static_cast<int>(row));
...
//----------------------------------------------------------------------------------------------
int LoadIsawPeaks::findPixelID(const Instrument_const_sptr& inst, std::string bankName,
                               int col, int row) {
