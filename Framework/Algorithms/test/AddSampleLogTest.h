...
public:
  ...
  template <typename T>
  void
  ExecuteAlgorithm(const MatrixWorkspace_sptr& testWS, const std::string& LogName,
                   const std::string& LogType, const std::string& LogText, T expectedValue,
                   bool fails = false, const std::string& LogUnit = "",
                   const std::string& NumberType = "AutoDetect", bool throws = false) {
    // add the workspace to the ADS
    AnalysisDataService::Instance().addOrReplace("AddSampleLogTest_Temporary",
                                                 testWS);
