...
private:
  ...
  void add_periods_logs(const WorkspaceGroup_sptr& ws, bool calculateNPeriods = true,
                        int nperiods = -1) {
...
public:
  ...
  void do_test_merging_two_workspaces_both_already_merged(const std::string& mergeType,
                                                          const std::string& result) {
    auto ws = create_group_workspace_with_sample_logs<double>(
        mergeType, "prop1", 1.0, 2.0, 3.0, 4.0);
