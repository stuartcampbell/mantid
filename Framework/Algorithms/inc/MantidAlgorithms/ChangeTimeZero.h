...
private:
  ...
  createOutputWS(const Mantid::API::MatrixWorkspace_sptr& input, double startProgress,
                 double stopProgress);
  /// Get the time shift
  double getTimeShift(API::MatrixWorkspace_sptr ws) const;
  /// Shift the time of the logs
  void shiftTimeOfLogs(const Mantid::API::MatrixWorkspace_sptr& ws, double timeShift,
