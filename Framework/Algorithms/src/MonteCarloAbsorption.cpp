...
void MonteCarloAbsorption::init() {
  ...
  declareProperty("CacheMeshElements", false,
                  "Control whether the calculation caches the mesh"
                  "elements that are intersected on the pre-scatter path"
                  "and on the post scatter path to a specific detector");
}
...
MatrixWorkspace_uptr MonteCarloAbsorption::doSimulation(
  ...
  const bool cacheMeshElements = getProperty("CacheMeshElements");
  ...
  for (int64_t i = 0; i < nhists; ++i) {
    ...
    /*for (int j = 0; j < nbins; j += lambdaStepSize) {
      ...
      std::tie(outY[j], std::ignore) = strategy.calculate(
          rng, detPos, lambdaIn, lambdaOut);
