...
ReflectometryReductionOne2::convertToQ(MatrixWorkspace_sptr inputWS) {
  ...
  auto thetaIn = getProperty("ThetaIn");
  bool const shouldCorrectAngle =
      !(*thetaIn).isDefault() && !summingInQ();
  if (shouldCorrectAngle && moreThanOneDetector) {
    auto ConvertSingleSpectrumLambdaToQ = this->createChildAlgorithm("ConvertSingleSpectrumLambdaToQ");
    ConvertSingleSpectrumLambdaToQ->initialize();
    ConvertSingleSpectrumLambdaToQ->setProperty("InputWorkspace", inputWS);
    ConvertSingleSpectrumLambdaToQ->setProperty("ThetaIn", thetaIn);
