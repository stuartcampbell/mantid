...
double RefinePowderInstrumentParameters3::calculateFunctionError(
    const IFunction_sptr& function, Workspace2D_sptr dataws, int wsindex) {
  ...
  bool fitOK =
      doFitFunction(function, std::move(dataws), wsindex, minimizer, 0, chi2, fitstatus);
 ...
 */
double RefinePowderInstrumentParameters3::fitFunction(const IFunction_sptr& function,
...
void RefinePowderInstrumentParameters3::addOrReplace(
    map<string, Parameter> &parameters, const string& parname, double parvalue) {
...
void storeFunctionParameterValue(
    const IFunction_sptr& function, map<string, pair<double, double>> &parvaluemap) {
  parvaluemap.clear();
...
void restoreFunctionParameterValue(
    map<string, pair<double, double>> parvaluemap, const IFunction_sptr& function,
    map<string, Parameter> &parammap) {
  vector<string> parnames = function->getParameterNames();
