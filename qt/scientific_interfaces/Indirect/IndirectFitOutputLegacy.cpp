...
MatrixWorkspace_sptr getMatrixWorkspaceFromGroup(const WorkspaceGroup_sptr& group,
                                                 std::size_t index) {
  if (group->size() > index)
...
IndirectFitOutputLegacy::IndirectFitOutputLegacy(
  ...
  addOutput(resultGroup, std::move(parameterTable), resultWorkspace, fitDataBegin,
            fitDataEnd);
}
...
IndirectFitOutputLegacy::IndirectFitOutputLegacy(
  ...
  addOutput(std::move(resultGroup), std::move(parameterTable), std::move(resultWorkspace), fitData, spectrum);
}
...
void IndirectFitOutputLegacy::addOutput(const WorkspaceGroup_sptr& resultGroup,
...
void IndirectFitOutputLegacy::updateParameters(
    ...
    const FitDataIteratorLegacy &fitDataBegin,
    const FitDataIteratorLegacy &fitDataEnd) {
  extractParametersFromTable(std::move(parameterTable), fitDataBegin, fitDataEnd,
                             m_parameters);
}
