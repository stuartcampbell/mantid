...
void IndirectFittingModel::addNewWorkspace(MatrixWorkspace_sptr workspace,
                                           const Spectra &spectra) {
  m_fittingData.emplace_back(new IndirectFitData(std::move(workspace), spectra));
...
void IndirectFittingModel::addSingleFitOutput(const IAlgorithm_sptr& fitAlgorithm,
...
IndirectFitOutput IndirectFittingModel::createFitOutput(
  ...
  return IndirectFitOutput(std::move(resultGroup), std::move(parameterTable), std::move(resultWorkspace),
                           fitDataBegin, fitDataEnd);
}
